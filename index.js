...
function filterByCountMinimum(animals, minimum) {
  ...
  for(let i = 0; i <= animals.length; i++) {
    if (animals[i] >= minimum) {
      minimumCount = animals[i]; // animals.count =
      //console.log();
    }
  }
    return minimumCount;
...
function getMostCommonAnimal(animals) {
  ...
  let highCount = animals.count[0];
  for (let i = 1; i < animals.length; i++){
    if (animals[i] > highCount) {
      highCount = animals[i];         // needs to return object with the highCount
      highestCountAnimal = animals[i].count
    }
